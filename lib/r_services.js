...
var reactLayoutHandler = function (options) {
  options.reply.view(options.jadeLayout, {
    layoutData: options.layoutData,
    layout: options.jadeLayout,
    reactComponent: React.renderToString(<LayoutComponent view={options.component} data={options.layoutData} />)
  });
};
    ...
    ctx.ops.forEach(function (op) {
      profile.getByUID(op, function (err, user) {
        ...
        if (count === ctx.ops.length) {
          ctx.ops = users;
          reactLayoutHandler({reply: reply, jadeLayout: 'index', component: IndexComponent, layoutData: ctx});
        }
      });
    });
  } else {
